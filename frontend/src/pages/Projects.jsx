  ...
  return (
    <div className="page">
      <header className="page-head">
        <div>
          <h1>Projects</h1>
          <p className="muted">All projects you own or belong to.</p>
        </div>
      ...
      {loading ? <p className="muted">Loading…</p> : projects.length === 0 ? (
        <EmptyState
          ...
          action={<button className="btn btn-primary" onClick={() => setOpen(true)}>Create project</button>}
        />
      ) : (
        <div className="proj-grid wide">
          {projects.map((p) => (
            <Link key={p._id || p.id} to={`/projects/${p._id || p.id}`} className="proj-card lg">
              ...
              <div className="grow">
                ...
                <div className="proj-meta">
                  ...
                  <span className="pill">{(p.tasksCount ?? p.tasks?.length ?? 0)} tasks</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Create project"
        ...
        <form onSubmit={create} className="form">
          <label className="field">
            <span>Name</span>
            <input value={form.name} onChange={(e)=>setForm({...form, name:e.target.value})} placeholder="Marketing site redesign" />
          </label>
          <label className="field">
